...
setwd("C:\\Users\\Administrador\\Desktop\\Maestría\\Datamining Ciencia y Tecnologia\\guia_TP_astronomia_2018")
...
```{r , echo=FALSE}
# Busqueda de estrellas idÃ©nticas en diferentes catalogos

# Carga de archivos en R
#
# Carga del catalogo Hipparcos
hip <- read_excel("hyades_source.xlsx", sheet = 2)

#Carga del Catalogo Symbad
symbad <- read_excel("hyades_source.xlsx", sheet = 1)

#Carga del Catalogo Tycho
tycho <- read_excel("hyades_source.xlsx", sheet = 3)
...
idcruz <- read_excel("id_cruzada_Symbad_Hipparcos.xlsx", col_names = c("HD","HIP"))
...
##Se analiza la distribución de los errores.
...
>Las distribuciones parecen ser normales excepto por la presencia de un outlier en ambos casos. La diferencia entre los valores de RA parecen tener menor dispersión que la distribución de las diferencias de los valores de DE.

>A continuación se plotean las Hyades en dos ejes de acuerdo a las diferencias presentan en su posición en ambos catálogos.
...
>En el siguiente gráfico se puede observar como en general el error de posicionamiento de la estrella en ambos catálogos se encuentra mayormente concentrado en el 6to decimal de la medición, excepto para la estrella 20441 que tiene su error en el 5to decimal.

>A continuación se filtra el dataframe para mostrar algunos de los casos reflejados en el gráfico anterior.
...
##En el siguiente gráfico a continuación se muestran las estrellas que fueron identificadas como Hyades efectivamente en el Catálogo Hipparcos.
...
## Nuestro vecindario estarÃ¡ delimitado por un cuadrado 3x3
...
## Tambien calculamos para las Hyades de Symbad su ubicaciÃ³n en la grilla
...
# El dataframe de estrellas mÃ¡s cercanas en Hipparcos a las de Symbad.
...
En el siguiente gráfico, se puede observar que las potenciales Hyades en el catálogo de Hipparcos poseen distancias demasiado pequeñas con respecto a su estrella candidata dentro del catálogo Symbad. Al separar los grupos en función de su distancia, teniendo en cuenta solamente distancias inferiores al 4 decimal como "verdaderas" Hyades, se puede observar que la población celeste se concentra en su totalidad dentro del 1er bin de la distribución.

A continuación se filtra el listado final de Hyades tomando como referencia las distancias inferiores al 4to decimal.
