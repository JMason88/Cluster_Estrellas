...
PAM_HIP_Euclid <- PAM_loop(x = hip1[,2:8] %>%
...
```{r}
PAM_HIP_Canberra <- PAM_loop(x = hip1[,2:8] %>%
...
PAM_HIP_Mahalanobis <- PAM_loop(x = hip1[,2:8] %>%
                            mahalanobis(., hip1_means, hip1_Sx),
                          data = hip1[,2:8],
                          k = 20)
...
fviz_pca_ind(prcomp(hip_clust1[,-c(1,9)]), 
...
PAM_TYC_Euclid <- PAM_loop(x = tyc1[,2:9] %>%
                            dist(., method = "euclidian"),
                          data = hip1[,2:9],
                          k = 20)
...
```{r}
PAM_TYC_Canberra <- PAM_loop(x = tyc1[,2:9] %>%
                            dist(., method = "canberra"),
                          data = tyc1[,2:9],
                          k = 20)
...
PAM_TYC_Mahalanobis <- PAM_loop(x = tyc1[,2:9] %>%
                            mahalanobis(., tyc1_means, tyc1_Sx),
                          data = hip1[,2:9],
                          k = 20)
