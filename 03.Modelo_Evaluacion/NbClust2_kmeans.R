NbClust2 <-function(data = NULL, diss=NULL, distance ="euclidean", min.nc=2, max.nc=15, method =NULL, index = "all", alphaBeale = 0.1, max.iter = 10)
{
    ...
    GAP <- function(X, cl, referenceDistribution, B, method, d, centrotypes) 
    {
      ...
      for (bb in (1:B)) {
        ...
        if (bb == 1) {
          ...
          { set.seed(1)
            pp2 <- kmeans(Xnew, ClassNr, 100, iter.max = max.iter)$cluster
          }
  ...
  {  
    ...
    {
      set.seed(1)
      cl2 <- kmeans(jeu,nc+1, iter.max = max.iter)$cluster
      set.seed(1)
      clmax <- kmeans(jeu,max_nc, iter.max = max.iter)$cluster
      if (nc > 2)
      {
        set.seed(1)
        cl1 <- kmeans(jeu,nc, iter.max = max.iter)$cluster
        clall <- cbind(cl1, cl2)
        set.seed(1)
        cl0 <- kmeans(jeu,nc-1, iter.max = max.iter)$cluster
        clall1 <- cbind(cl0, cl1, cl2)
      }
      if (nc == 2)
      {
  ...
  if(any(indice == 31) || (indice == 32))
  {
    ...
    {
      set.seed(1)
      partition<-kmeans(jeu,j, iter.max = max.iter)$cluster
    }
      ...
      ||(indice==28)||(indice==30))
  {
    ...
    {
      set.seed(1)
      partition<-kmeans(jeu,best.nc, iter.max = max.iter)$cluster
    }
